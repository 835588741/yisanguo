...
import com.bssy.datepick.widget.NumericWheelAdapter;
import com.bssy.datepick.widget.OnWheelChangedListener;
import com.bssy.datepick.widget.WheelView;

/**
 *  日期选择控件
 *  birth = new SelectBirthday(MainActivity.this);
 *	birth.showAtLocation(MainActivity.this.findViewById(R.id.root),Gravity.BOTTOM, 0, 0);
 * @file XSYDatePickView.java
 * @author 许仕永(xsy)
 * @package_name com.bssy.customui.widget
 * @todo  TODO
 * @date 2015-12-25 下午2:49:29
 */
public class XSYDatePickView extends PopupWindow implements OnClickListener {
	...
	private TextView txtShowView;

	public XSYDatePickView(Activity context,TextView txtShowView) {
		super(context);
		mContext = context;
		
		this.txtShowView = txtShowView;
		Calendar c = Calendar.getInstance();  
		int xyear = c.get(Calendar.YEAR);  
		int xmonth = c.get(Calendar.MONTH);  
		int xday = c.get(Calendar.DAY_OF_MONTH);  
		
		this.dateString = xyear+"-"+(xmonth+1)+"-"+xday;//"2012-9-25";
	...
	private void updateDays(WheelView year, WheelView month, WheelView day) {
		...
		txtShowView.setText(dateString);
	}
	...
	public String getValueOfDateString()
	{
		return dateString;
	}
